    ...
    <form method="POST" action="{{ route('settings.update') }}" enctype="multipart/form-data" class="bg-white rounded-xl border border-gray-200 overflow-hidden">
        ...
        <div class="p-6 space-y-6">
            ...
            <div class="border-t border-gray-200 pt-6">
                ...
                <p class="text-xs text-gray-500 mb-4">Tarif yang dibebankan ke pagu kegiatan untuk setiap pegawai per tanggal pemakaian. Perubahan tarif hanya berlaku untuk perjalanan dinas baru, data lama tetap memakai tarif saat dibuat.</p>
                <div>
                    <label for="tarif_perjalanan_dinas_display" class="block text-sm font-medium text-gray-700 mb-1">Tarif per Orang per Hari (Rp)</label>
                    <div class="relative">
                        <span class="absolute inset-y-0 left-0 pl-3 flex items-center text-sm text-gray-500 pointer-events-none">Rp</span>
                        <input type="text" id="tarif_perjalanan_dinas_display"
                               value="{{ number_format((float) old('tarif_perjalanan_dinas', $settings['tarif_perjalanan_dinas']), 0, ',', '.') }}"
                               oninput="formatTarifPerjalananDinas(this)"
                               class="w-full pl-10 rounded-lg border-gray-300 text-sm focus:border-indigo-500 focus:ring-indigo-500"
                               placeholder="80.000" inputmode="numeric" autocomplete="off">
                        <input type="hidden" id="tarif_perjalanan_dinas" name="tarif_perjalanan_dinas"
                               value="{{ old('tarif_perjalanan_dinas', $settings['tarif_perjalanan_dinas']) }}">
                    </div>
                    <p class="text-xs text-gray-400 mt-1">Default: Rp 80.000. Nilai ini dipakai sebagai pengurang otomatis pagu kegiatan saat menambahkan perjalanan dinas baru.</p>
                </div>
            </div>
...
@section('scripts')
<script>
function formatTarifPerjalananDinas(el) {
    const numeric = (el.value || '').replace(/[^0-9]/g, '');
    const hidden = document.getElementById('tarif_perjalanan_dinas');
    if (hidden) hidden.value = numeric;
    el.value = numeric === '' ? '' : Number(numeric).toLocaleString('id-ID');
}
